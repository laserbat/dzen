...
static void
qsi_no_xinerama(XRectangle *rect) {
    rect->x = 0;
    rect->y = 0;
...
main(int argc, char *argv[]) {
    ...
    for(i = 1; i < argc; i++) {
        ...
        if(!strncmp(argv[i], "-bg", 4)) {
            if(++i < argc) XParseColor(dzen.dpy, cmap, argv[i], &(dzen.bg));
            continue;
        }
        if(!strncmp(argv[i], "-fg", 4)) {
            if(++i < argc) XParseColor(dzen.dpy, cmap, argv[i], &(dzen.fg));
            continue;
        }
